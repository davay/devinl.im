...
<div class="max-w-2xl font-mono text-xs">
	<div class="mb-5 flex flex-wrap items-baseline gap-x-3 gap-y-1 border-b border-gray-250 pb-4">
		...
		{#each tags as tag (tag)}
			<a href="{basePath}?tag={tag}" class="hover:text-black {activeTag === tag ? 'text-black' : 'text-gray-450'}">{activeTag === tag ? `[#${tag}]` : `#${tag}`}</a>
		{/each}
	</div>
	...
	{#each items as item (item.slug)}
		<a href="{basePath}/{item.slug}" class="group mb-4 block">
			...
			{#if item.description}
				<div class="mt-1 text-gray-450">{item.description}</div>
			{/if}
		</a>
	{/each}
</div>
